...
try {
  // Get test business
  const business = db.prepare('SELECT * FROM businesses ORDER BY id DESC LIMIT 1').get();
  
  if (!business) {
    console.log('❌ No business found. Run create-test-business.js first.');
    process.exit(1);
  }
  ...
  console.log('   • Business Analytics: http://localhost:3000/dashboard/analytics/' + business.id);
  console.log('   • Admin Analytics: http://localhost:3000/admin/analytics');
  console.log('');
  console.log('💡 The data will be aggregated automatically when you view the dashboards!');
