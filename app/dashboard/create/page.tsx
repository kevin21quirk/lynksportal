...
export default function CreateBusinessPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          whatsappNumber: data.whatsappNumber || formData.whatsappNumber,
          messengerUrl: data.messengerUrl || formData.messengerUrl,
          address: data.address || formData.address,
          logoUrl: data.logoUrl || formData.logoUrl,
          heroImageUrl: data.heroImageUrl || formData.heroImageUrl,
        ...
        // Update business hours if found
        if (data.businessHours && data.businessHours.length > 0) {
          setBusinessHours(data.businessHours);
        }

        alert('Business details, images, services, hours, and gallery auto-filled from website!');
      } else {
  ...
  const handleImageUpload = (field: 'logoUrl' | 'heroImageUrl' | 'coverImageUrl', file: File) => {
    // For now, use a placeholder URL. In production, upload to cloud storage
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save services as JSON in businesses table
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0c0f17' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2" style={{ color: '#dbf72c' }}>Create Business Page</h1>
          <p className="text-white">Build your custom business page with social links</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              ...
              {services.length === 0 && (
                <p className="text-gray-400 text-center py-4">
                  No services added yet. Click "Add Service" or use AI Scan!
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              ...
              {galleryImages.length === 0 && (
                <p className="text-gray-400 text-center py-4">
                  No gallery images added yet. Click "Add Image" or use AI Scan!
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-4">
            ...
            >
              <Save size={20} />
              <span>{loading ? 'Creating...' : 'Create Business Page'}</span>
            </button>
          </div>
        </form>
      </div>
