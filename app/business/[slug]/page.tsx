...
export default function BusinessPage() {
  ...
  // Use actual gallery images only (no fallbacks)
  const displayGalleryImages = galleryImages;

  return (
    <div className="min-h-screen py-8" style={{ backgroundColor: '#0c0f17' }}>
      <div className="max-w-md mx-auto px-4">
        <div className="bg-black rounded-3xl shadow-2xl overflow-hidden p-6 space-y-6">
          ...
          {isRestaurant && displayGalleryImages.length > 0 && (
            <div>
              <h2 className="text-white text-lg font-bold mb-3">OUR MENUS</h2>
              <div className="bg-white rounded-xl p-6">
                <div className="grid grid-cols-2 gap-4">
                  {displayGalleryImages.slice(0, 4).map((img, index) => (
                    <div key={index} className="aspect-square rounded-lg overflow-hidden">
                      <img 
                        src={img.url} 
                        alt={img.caption || `Menu ${index + 1}`}
              ...
              <div className="bg-white rounded-xl p-6">
                ...
                <div className="rounded-lg overflow-hidden h-48">
                  <iframe
                    ...
                    frameBorder="0"
                    style={{ border: 0 }}
                    src={`https://www.google.com/maps?q=${encodeURIComponent(business.address + (business.city ? ', ' + business.city : '') + (business.postcode ? ' ' + business.postcode : ''))}&output=embed`}
                    allowFullScreen
                  ></iframe>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {businessHours.map((hours, index) => (
                    <div key={index} className="flex justify-between">
                      <span className="text-gray-900 font-medium">{hours.day_of_week}</span>
